...
export default {
    ...
    reWriteUrl(options) {
        ...
        if (!options.isUseDefaultIp) {
            if (urlConfig[process.env.NODE_PRODUCT]) {

                urlConfig[process.env.NODE_PRODUCT](options);
        ...
        if (process.env.NODE_ENV == 'test' || process.env.NODE_ENV == 'mocktest') { //用的是测试环境的数据
            ...
            if (options.api) {
                ...
                options.url = `http://47.96.12.218:8070${options.url}`;
            } else if (options.cdn) {

                options.url = `http://47.96.12.218:8070/v3/getRecommendation`;
                options.method = 'POST';
            } else {

                options.url = `http://47.96.12.218:8030${options.url}`;
            }
        }
